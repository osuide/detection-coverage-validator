...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "role",
            postgresql.ENUM(
                "owner", "admin", "member", "viewer", name="userrole", create_type=False
            ),
            default="member",
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_org_members_invite_token", "organization_members", ["invite_token"]
    )
    ...
    op.create_index(
        "ix_sessions_user_active", "user_sessions", ["user_id", "is_active"]
    )
