...
def upgrade() -> None:
    # Create enums using raw SQL with exception handling for idempotency
    # This handles the case where a previous migration attempt created the enum
    # but then failed, leaving the enum in place without completing the migration

    # Create drift_severity enum
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE driftseverity AS ENUM ('none', 'info', 'warning', 'critical');
        EXCEPTION
    ...
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE customdetectionformat AS ENUM (
                'sigma', 'yara', 'snort', 'suricata', 'spl',
                'kql', 'elasticsearch', 'cloudwatch', 'custom'
            );
        EXCEPTION
            WHEN duplicate_object THEN null;
        END $$;
    ...
    op.execute(
        """
        DO $$ BEGIN
            CREATE TYPE customdetectionstatus AS ENUM (
                'pending', 'processing', 'mapped', 'failed', 'needs_review'
            );
        EXCEPTION
            WHEN duplicate_object THEN null;
        END $$;
    ...
    # Create coverage_history table (with IF NOT EXISTS for idempotency)
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    existing_tables = inspector.get_table_names()

    if "coverage_history" not in existing_tables:
        op.create_table(
            "coverage_history",
    ...
    if "coverage_alerts" not in existing_tables:
        op.create_table(
            "coverage_alerts",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
    ...
    if "custom_detections" not in existing_tables:
        op.create_table(
            "custom_detections",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column(
            ...
            sa.Column(
                "mapped_techniques", postgresql.ARRAY(sa.String()), nullable=True
            ),
    ...
    if "custom_detection_batches" not in existing_tables:
        op.create_table(
            ...
            sa.Column(
                "processed_rules", sa.Integer(), nullable=False, server_default="0"
            ),
            sa.Column(
                "successful_rules", sa.Integer(), nullable=False, server_default="0"
            ),
...
def downgrade() -> None:
    # Drop tables (IF EXISTS for idempotency)
    op.execute("DROP TABLE IF EXISTS custom_detection_batches CASCADE")
    op.execute("DROP TABLE IF EXISTS custom_detections CASCADE")
    op.execute("DROP TABLE IF EXISTS coverage_alerts CASCADE")
    op.execute("DROP TABLE IF EXISTS coverage_history CASCADE")

    # Drop enums (IF EXISTS for idempotency)
