...
data "aws_iam_policy_document" "eventbridge_dlq_policy" {
  statement {
    ...
    condition {
      ...
      values   = [aws_cloudwatch_event_rule.suspicious_lambda.arn]
    }
  }
}
...
resource "google_monitoring_alert_policy" "external_container_alert" {
  ...
  documentation {
    ...
    EOT
        ...
        # Azure Strategy: User Execution
        DetectionStrategy(
            strategy_id="t1204-azure",
            name="Azure User Execution Detection",
            description=(
                "Azure detection for User Execution. "
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "detection" {
  ...
  criteria {
    query = <<-QUERY
// Sentinel Analytics Rule: User Execution
// MITRE ATT&CK: T1204
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
  ...
  description = "Detects User Execution (T1204) activity in Azure environment"
  display_name = "User Execution Detection"
  enabled      = true
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.detection.id
                ...
                alert_severity="high",
                alert_title="Azure: User Execution Detected",
                alert_description_template=(
                    "User Execution activity detected. "
