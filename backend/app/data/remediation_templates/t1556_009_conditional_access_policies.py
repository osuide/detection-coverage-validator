...
resource "aws_sqs_queue_policy" "dlq" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ArnEquals = { "aws:SourceArn" = aws_cloudwatch_event_rule.scp_changes.arn }
      }
    }]
  })
}

# EventBridge target to SNS with retry and DLQ
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            ...
            ArnEquals = {
              "aws:SourceArn" = aws_cloudwatch_event_rule.scp_changes.arn
            }
        }
      }
    ]
  ...
  alert_strategy {
    ...
    notification_rate_limit {
      period = "300s"
        ...
        # Azure Strategy: Modify Authentication Process: Conditional Access Policies
        DetectionStrategy(
            strategy_id="t1556009-azure",
            name="Azure Modify Authentication Process: Conditional Access Policies Detection",
            description=(
                "Azure detection for Modify Authentication Process: Conditional Access Policies. "
...
resource "azurerm_monitor_action_group" "defender_alerts" {
  name                = "defender-t1556-009-alerts"
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.defender_detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Modify Authentication Process: Conditional Access Policies Detected",
                alert_description_template=(
                    "Modify Authentication Process: Conditional Access Policies activity detected. "
