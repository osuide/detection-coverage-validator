...
data "aws_iam_policy_document" "eventbridge_dlq_policy" {
  statement {
    ...
    condition {
      ...
      values   = [aws_cloudwatch_event_rule.ecs_exec.arn]
    }
  }
}
  ...
  alert_strategy {
    ...
    notification_rate_limit {
      period = "300s"
        ...
        # Azure Strategy: Remote Services
        DetectionStrategy(
            strategy_id="t1021-azure",
            name="Azure Remote Services Detection",
            description=(
                "Sentinel detects anomalous remote service usage. "
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "detection" {
  name                = "remote-services-detection"
  resource_group_name = var.resource_group_name
  ...
  criteria {
    query = <<-QUERY
// Sentinel Analytics Rule: Remote Services
// MITRE ATT&CK: T1021
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
  ...
  description = "Detects Remote Services (T1021) activity in Azure environment"
  display_name = "Remote Services Detection"
  enabled      = true
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Remote Services Detected",
                alert_description_template=(
                    "Remote Services activity detected. "
