...
Resources:
  ...
  TopicPolicy:
    ...
    Properties:
      Topics: [!Ref RootAccountAlertTopic]
      PolicyDocument:
        Version: '2012-10-17'
        Statement:
          - Sid: AllowCloudWatchPublish
            ...
            Action: sns:Publish
            Resource: !Ref RootAccountAlertTopic
            Condition:
              StringEquals:
...
Resources:
  ...
  TopicPolicy:
    Type: AWS::SNS::TopicPolicy
    Properties:
      Topics: [!Ref ServiceAccountAlertTopic]
      PolicyDocument:
        Version: '2012-10-17'
        Statement:
          - Sid: AllowCloudWatchPublish
            ...
            Principal:
              Service: cloudwatch.amazonaws.com
            Action: sns:Publish
            Resource: !Ref ServiceAccountAlertTopic
            Condition:
              StringEquals:
                AWS:SourceAccount: !Ref AWS::AccountId""",
...
resource "aws_sns_topic_policy" "allow_cloudwatch" {
  arn = aws_sns_topic.service_account_alerts.arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid       = "AllowCloudWatchPublish"
      ...
      Principal = { Service = "cloudwatch.amazonaws.com" }
      Action    = "sns:Publish"
      Resource  = aws_sns_topic.service_account_alerts.arn
      Condition = {
        StringEquals = {
          "AWS:SourceAccount" = data.aws_caller_identity.current.account_id
        }
        ...
        Statement:
          - Sid: AllowCloudWatchPublish
            ...
            Principal:
              Service: cloudwatch.amazonaws.com
            Action: sns:Publish
            Resource: !Ref RootSSHAlertTopic
            Condition:
              StringEquals:
                AWS:SourceAccount: !Ref AWS::AccountId""",
...
resource "aws_sns_topic_policy" "allow_cloudwatch" {
  arn = aws_sns_topic.root_ssh_alerts.arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid       = "AllowCloudWatchPublish"
      ...
      Principal = { Service = "cloudwatch.amazonaws.com" }
      Action    = "sns:Publish"
      Resource  = aws_sns_topic.root_ssh_alerts.arn
      Condition = {
        StringEquals = {
          "AWS:SourceAccount" = data.aws_caller_identity.current.account_id
        }
...
resource "aws_sns_topic_subscription" "email" {
  ...
  endpoint  = var.alert_email
        ...
        # Azure Strategy: Valid Accounts: Default Accounts
        DetectionStrategy(
            strategy_id="t1078001-azure",
            name="Azure Valid Accounts: Default Accounts Detection",
            description=(
                "Azure detection for Valid Accounts: Default Accounts. "
...
# Microsoft Defender detects Valid Accounts: Default Accounts activity
...
resource "azurerm_monitor_action_group" "defender_alerts" {
  name                = "defender-t1078-001-alerts"
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.defender_detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Valid Accounts: Default Accounts Detected",
                alert_description_template=(
                    "Valid Accounts: Default Accounts activity detected. "
