        ...
        Statement:
          - Effect: Allow
            ...
            Condition:
              ...
              ArnEquals:
                aws:SourceArn: !GetAtt NetworkConfigRule.Arn""",
...
resource "aws_sqs_queue_policy" "dlq" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ArnEquals = { "aws:SourceArn" = aws_cloudwatch_event_rule.network_config.arn }
      }
    }]
  })
}
  ...
  policy = jsonencode({
    ...
    Condition = {
        StringEquals = {
          ...
          ArnEquals = {
            "aws:SourceArn" = aws_cloudwatch_event_rule.network_config.arn
          }
      }
    }]
  })
...
resource "google_compute_packet_mirroring" "arp_detection" {
  ...
  filter {
    ...
    direction    = "BOTH"
        ...
        # Azure Strategy: Adversary-in-the-Middle: ARP Cache Poisoning
        DetectionStrategy(
            strategy_id="t1557002-azure",
            name="Azure Adversary-in-the-Middle: ARP Cache Poisoning Detection",
            description=(
                "Azure detection for Adversary-in-the-Middle: ARP Cache Poisoning. "
...
# Microsoft Defender detects Adversary-in-the-Middle: ARP Cache Poisoning activity
...
resource "azurerm_monitor_action_group" "defender_alerts" {
  name                = "defender-t1557-002-alerts"
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.defender_detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Adversary-in-the-Middle: ARP Cache Poisoning Detected",
                alert_description_template=(
                    "Adversary-in-the-Middle: ARP Cache Poisoning activity detected. "
