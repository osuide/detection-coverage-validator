...
resource "aws_sns_topic_policy" "allow_cloudwatch" {
  arn = aws_sns_topic.bits_alerts.arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid       = "AllowCloudWatchPublish"
      ...
      Action    = "sns:Publish"
      Resource  = aws_sns_topic.bits_alerts.arn
      Condition = {
        StringEquals = {
...
resource "aws_sns_topic_policy" "allow_cloudwatch" {
  arn = aws_sns_topic.bits_event_alerts.arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid       = "AllowCloudWatchPublish"
      ...
      Principal = { Service = "cloudwatch.amazonaws.com" }
      Action    = "sns:Publish"
      Resource  = aws_sns_topic.bits_event_alerts.arn
      Condition = {
        StringEquals = {
          "AWS:SourceAccount" = data.aws_caller_identity.current.account_id
        }
...
resource "aws_sns_topic_policy" "allow_cloudwatch" {
  arn = aws_sns_topic.bits_network_alerts.arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid       = "AllowCloudWatchPublish"
      ...
      Principal = { Service = "cloudwatch.amazonaws.com" }
      Action    = "sns:Publish"
      Resource  = aws_sns_topic.bits_network_alerts.arn
      Condition = {
        StringEquals = {
          "AWS:SourceAccount" = data.aws_caller_identity.current.account_id
        }
...
resource "google_monitoring_alert_policy" "bits_events" {
  ...
  documentation {
    ...
    mime_type = "text/markdown"
        ...
        # Azure Strategy: BITS Jobs
        DetectionStrategy(
            strategy_id="t1197-azure",
            name="Azure BITS Jobs Detection",
            description=(
                "Azure detection for BITS Jobs. "
            ...
            implementation=DetectionImplementation(
                sentinel_rule_query="""// Sentinel Analytics Rule: BITS Jobs
// MITRE ATT&CK: T1197
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
    ...
    Operations,
    Resources""",
                azure_terraform_template="""# Azure Detection for BITS Jobs
# MITRE ATT&CK: T1197
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "detection" {
  ...
  criteria {
    query = <<-QUERY
// Sentinel Analytics Rule: BITS Jobs
// MITRE ATT&CK: T1197
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
  ...
  description = "Detects BITS Jobs (T1197) activity in Azure environment"
  display_name = "BITS Jobs Detection"
  enabled      = true
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.detection.id
                ...
                alert_severity="high",
                alert_title="Azure: BITS Jobs Detected",
                alert_description_template=(
                    "BITS Jobs activity detected. "
