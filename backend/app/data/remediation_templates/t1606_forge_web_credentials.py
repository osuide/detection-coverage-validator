...
resource "aws_sns_topic_policy" "saml_alerts" {
...
Resources:
  ...
  TopicPolicy:
    Type: AWS::SNS::TopicPolicy
    Properties:
      Topics: [!Ref FederationAlertTopic]
      PolicyDocument:
        Version: '2012-10-17'
        Statement:
          - Sid: AllowCloudWatchAlarms
            ...
            Principal:
              Service: cloudwatch.amazonaws.com
            Action: sns:Publish
            Resource: !Ref FederationAlertTopic
            Condition:
              StringEquals:
                AWS:SourceAccount: !Ref AWS::AccountId""",
...
resource "aws_sns_topic_policy" "federation_alerts" {
  arn = aws_sns_topic.federation_alerts.arn
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Sid       = "AllowCloudWatchAlarms"
      ...
      Principal = { Service = "cloudwatch.amazonaws.com" }
      Action    = "sns:Publish"
      Resource  = aws_sns_topic.federation_alerts.arn
      Condition = {
        StringEquals = {
          "AWS:SourceAccount" = data.aws_caller_identity.current.account_id
        }
        ...
        Statement:
          - Sid: AllowCloudWatchAlarms
            ...
            Principal:
              Service: cloudwatch.amazonaws.com
            Action: sns:Publish
            Resource: !Ref SessionAlertTopic
            Condition:
              StringEquals:
                AWS:SourceAccount: !Ref AWS::AccountId""",
...
resource "aws_sns_topic_policy" "session_alerts" {
  ...
  alert_strategy {
    ...
    notification_rate_limit {
      period = "300s"
        ...
        # Azure Strategy: Forge Web Credentials
        DetectionStrategy(
            strategy_id="t1606-azure",
            name="Azure Forge Web Credentials Detection",
            description=(
                "Azure detection for Forge Web Credentials. "
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.defender_detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Forge Web Credentials Detected",
                alert_description_template=(
                    "Forge Web Credentials activity detected. "
