...
resource "aws_sqs_queue_policy" "dlq_policy" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Condition = {
        ArnEquals = {
          "aws:SourceArn" = aws_cloudwatch_event_rule.leave_org.arn
        }
      }
    }]
  })
...
resource "aws_sqs_queue" "move_dlq" {
  name                      = "move-account-dlq"
  message_retention_seconds = 1209600  # 14 days
}

resource "aws_sqs_queue_policy" "move_dlq_policy" {
  queue_url = aws_sqs_queue.move_dlq.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [{
      Effect    = "Allow"
      Principal = { Service = "events.amazonaws.com" }
      Action    = "sqs:SendMessage"
      Resource  = aws_sqs_queue.move_dlq.arn
      Condition = {
        ArnEquals = {
          "aws:SourceArn" = aws_cloudwatch_event_rule.move_account.arn
        }
      }
    }]
  })
...
resource "aws_cloudwatch_event_target" "sns" {
  ...
  dead_letter_config {
    arn = aws_sqs_queue.move_dlq.arn
  }
  input_transformer {
    input_paths = {
      account       = "$.account"
  ...
  alert_strategy {
    ...
    notification_rate_limit {
      period = "300s"
        ...
        # Azure Strategy: Modify Cloud Resource Hierarchy
        DetectionStrategy(
            strategy_id="t1666-azure",
            name="Azure Modify Cloud Resource Hierarchy Detection",
            description=(
                "Azure detection for Modify Cloud Resource Hierarchy. "
            ...
            implementation=DetectionImplementation(
                sentinel_rule_query="""// Sentinel Analytics Rule: Modify Cloud Resource Hierarchy
// MITRE ATT&CK: T1666
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
    ...
    Operations,
    Resources""",
                azure_terraform_template="""# Azure Detection for Modify Cloud Resource Hierarchy
# MITRE ATT&CK: T1666
...
resource "azurerm_monitor_action_group" "security_alerts" {
  name                = "modify-cloud-resource-hierarchy-alerts"
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "detection" {
  name                = "modify-cloud-resource-hierarchy-detection"
  resource_group_name = var.resource_group_name
  location            = "uksouth"
  ...
  criteria {
    query = <<-QUERY
// Sentinel Analytics Rule: Modify Cloud Resource Hierarchy
// MITRE ATT&CK: T1666
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
  ...
  description = "Detects Modify Cloud Resource Hierarchy (T1666) activity in Azure environment"
  display_name = "Modify Cloud Resource Hierarchy Detection"
  enabled      = true

  tags = {
    "mitre-technique" = "T1666"
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Modify Cloud Resource Hierarchy Detected",
                alert_description_template=(
                    "Modify Cloud Resource Hierarchy activity detected. "
