        ...
        Statement:
          - Effect: Allow
            ...
            Condition:
              ...
              ArnEquals:
                aws:SourceArn: !GetAtt CertificateRule.Arn""",
          ...
          - Effect: Allow
            ...
            Condition:
              StringEquals:
                AWS:SourceAccount: !Ref AWS::AccountId
              ArnEquals:
                aws:SourceArn: !GetAtt GuardDutyEncryptedC2Rule.Arn""",
  ...
  policy = jsonencode({
    ...
    Condition = {
        StringEquals = {
          ...
          ArnEquals = {
            "aws:SourceArn" = aws_cloudwatch_event_rule.guardduty_encrypted_c2.arn
          }
      }
    }]
  })
  ...
  logging_configuration {
    log_destination_config {
      ...
      log_type             = "ALERT"
        ...
        # Azure Strategy: Encrypted Channel
        DetectionStrategy(
            strategy_id="t1573-azure",
            name="Azure Encrypted Channel Detection",
            description=(
                "Azure detection for Encrypted Channel. "
            ...
            implementation=DetectionImplementation(
                sentinel_rule_query="""// Sentinel Analytics Rule: Encrypted Channel
// MITRE ATT&CK: T1573
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
    ...
    Operations,
    Resources""",
                azure_terraform_template="""# Azure Detection for Encrypted Channel
# MITRE ATT&CK: T1573
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "detection" {
  name                = "encrypted-channel-detection"
  resource_group_name = var.resource_group_name
  location            = "uksouth"
  ...
  criteria {
    query = <<-QUERY
// Sentinel Analytics Rule: Encrypted Channel
// MITRE ATT&CK: T1573
let lookback = 24h;
let threshold = 5;
AzureActivity
...
| project
  ...
  description = "Detects Encrypted Channel (T1573) activity in Azure environment"
  display_name = "Encrypted Channel Detection"
  enabled      = true

  tags = {
    "mitre-technique" = "T1573"
...
output "alert_rule_id" {
  value = azurerm_monitor_scheduled_query_rules_alert_v2.detection.id
                ...
                alert_severity="high",
                alert_title="Azure: Encrypted Channel Detected",
                alert_description_template=(
                    "Encrypted Channel activity detected. "
